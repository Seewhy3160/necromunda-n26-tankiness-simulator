...
/* Enemy credits to Down for the reference fighters: the enemy's cheapest tool. */
{
  ...
  const rows = fighters.map(([name, p, w, c]) => {
    ...
    return [name, String(c), String(Math.round(r.enemyCredits)), String(Math.round(r.enemyCreditsPer100)),
      r.plan.steps.map(s => `${s.name.split(' (')[0].split(' / ')[0]} ${Math.round(s.share * 100)}%`).join(', '), r.hitsToDown.toFixed(2)];
  });
  sections.enemyCredits = table(['Fighter', 'Cost', 'Enemy credits to Down', 'Per 100 credits', 'Enemy’s cheapest plan', 'Hits to Down (mix)'], rows);
}
...
/* The attacker packages behind the figures. */
{
  const rows = T.POOL_V1.profiles.map(p => [p.name, p.attacker.carrier, String(p.attacker.weaponCost), String(p.attacker.cost),
    p.attacker.hitsPerBattle.toFixed(2), String(p.attacker.count), p.attacker.basis]);
  sections.attackers = table(['Weapon', 'Carrier', 'Weapon (credits)', 'Package (credits)', 'Hits per battle', 'In the reference gang', 'Basis'], rows);
}
